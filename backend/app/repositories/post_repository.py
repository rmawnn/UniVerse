...
class PostRepository:
    ...
    async def search(
        ...
        *,
        ...
        """Search non-deleted posts by content (ILIKE).

        Ranking: exact phrase first, then by engagement (likes + comments),
        then newest first.
        """
        from app.models.community import Community
        ...
        engagement = (
            func.coalesce(like_counts.c.like_cnt, 0)
            + func.coalesce(comment_counts.c.comment_cnt, 0)
        )
    ...
    async def count_search(self, query: str) -> int:
        """Count non-deleted posts matching the search query."""
        from app.models.community import Community
