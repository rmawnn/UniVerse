...
import { listFollowing, getUserProfile } from "@/api/users";
...
export default function FollowingPage({
  ...
  const followingQuery = useQuery({
    ...
    queryFn: () => listFollowing(userId, { page_size: 100 }),
    staleTime: 30_000,
  });
    ...
    <div>
      ...
      {followingQuery.isError && (
        <div style={styles.error}>
          <span>Could not load following list.</span>
          <button onClick={() => followingQuery.refetch()} style={styles.retryBtn}>
            Retry
          </button>
        </div>
      )}
      ...
      {!followingQuery.isLoading && !followingQuery.isError && following.length === 0 && (
        <div style={styles.empty}>
          ...
          <p style={styles.emptyTitle}>Not following anyone yet</p>
          <p style={styles.emptyHint}>When this user follows people, they&apos;ll appear here.</p>
        </div>
      )}
